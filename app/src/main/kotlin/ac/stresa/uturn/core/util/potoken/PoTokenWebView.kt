...
//            expirationInstant = Instant.now().plusSeconds(expirationTimeInSeconds - 600)
    ...
    override fun isExpired(): Boolean {
        return Clock.System.now() > expirationInstant
    }
    ...
    companion object : PoTokenGenerator.Factory {
        ...
        private fun runOnMainThread(emitterIfPostFails: SingleEmitter<out Any>, runnable: Runnable) {
            if (!Handler(Looper.getMainLooper()).post(runnable)) emitterIfPostFails.onError(PoTokenException("Could not run on main thread"))
        }
    }
}
